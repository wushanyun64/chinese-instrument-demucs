...
```bash
uv run python -m demucs --repo ./release_models -n SIG --two-stems chinese-instrument input.wav
```
...
# Batch folder
uv run python inference/separate.py input_folder/ --sig <SIG> --stem erhu
```
