...
from data_pipeline.validate_flute_free import _flute_band_ratio, validate
...
class TestMixAtSNR:
    def test_positive_snr_flute_louder(self):
        # Use uncorrelated signals so powers add, not amplitudes
        torch.manual_seed(42)
        flute = torch.randn(2, 1000)
        other = torch.randn(2, 1000)
        mixture = mix_at_snr(flute, other, snr_db=10.0)
        # At SNR=10dB: P_other_scaled = P_flute / 10
        p_flute = flute.square().mean()
        p_other_scaled = (mixture - flute).square().mean()
        ratio = p_flute / p_other_scaled
        # Should be ~10 (10 dB)
        assert 8.0 < ratio.item() < 12.0

    def test_negative_snr_flute_quieter(self):
        torch.manual_seed(42)
        flute = torch.randn(2, 1000)
        other = torch.randn(2, 1000)
        mixture = mix_at_snr(flute, other, snr_db=-10.0)
...
class TestBuildTrack:
    def test_invariant_mixture_equals_sum(self, tmp_path: Path):
        """The plan's key invariant: mixture == flute + other sample-exact."""
        # Create synthetic clips
        flute_file = tmp_path / "flute.wav"
        bg_file = tmp_path / "bg.wav"
        _make_wav_file(flute_file, _sine_tone(440, seconds=3.0))
        ...
        expected = stems["chinese-flute"] + stems["other"]

        max_diff = (mixture - expected).abs().max().item()
        assert max_diff < 1e-6, f"mixture ≠ flute+other (max diff: {max_diff})"
    ...
    def test_output_is_stereo(self, tmp_path: Path):
        ...
        for name in ("mixture", "chinese-flute", "other"):
...
class TestBuildDataset:
    def test_creates_correct_structure(self, tmp_path: Path):
        flute_dir = tmp_path / "flute"
        bg_dir = tmp_path / "bg"
        out_dir = tmp_path / "dataset"
        for d in (flute_dir, bg_dir):
            d.mkdir()

        # 5 flute clips, 5 backgrounds — use short segments
        for i in range(5):
            _make_wav_file(flute_dir / f"flute_{i}.wav", _sine_tone(440 + i * 50, seconds=1.5))
    ...
    def test_train_valid_no_shared_flute_clips(self, tmp_path: Path):
        ...
        for i in range(3):
            _make_wav_file(flute_dir / f"flute_{i}.wav", _sine_tone(440 + i * 50, seconds=0.5))
        for i in range(3):
            _make_wav_file(bg_dir / f"bg_{i}.wav", _white_noise(seconds=0.5))

        build_dataset(
            flute_dir=flute_dir, bg_dir=bg_dir, out_dir=out_dir,
            num_train=2, num_valid=1, seg_len=0.2,
        )

        # We can't directly verify file identity from the output since we use
        # segments, but we CAN verify no IDENTICAL flute source files appear
        # in both splits by checking that the flute stems differ meaningfully.
        # (Per-plan, split is by source clip identity — same flute file
        #  never contributes to both train and valid.)
        train_flutes = []
        valid_flutes = []
        import soundfile as sf
        for split, store in [("train", train_flutes), ("valid", valid_flutes)]:
            for track_dir in (out_dir / split).iterdir():
                data, _ = sf.read(str(track_dir / "chinese-flute.wav"))
                wav = torch.from_numpy(data.T)
                store.append(wav)

        # Every valid flute should differ from every train flute
        for vf in valid_flutes:
            for tf in train_flutes:
                assert not torch.allclose(vf, tf), "Train/valid share a flute clip!"


# ---------------------------------------------------------------------------
# validate_flute_free tests
# ---------------------------------------------------------------------------

class TestFluteBandRatio:
    def test_sine_vs_noise(self):
        """A sine in the flute band should have a higher ratio than white noise."""
        sine = _sine_tone(1000, seconds=3.0)  # middle of flute band
        noise = _white_noise(seconds=3.0, stereo=False)

        ratio_sine = _flute_band_ratio(sine)
        ratio_noise = _flute_band_ratio(noise)
        assert ratio_sine > ratio_noise, (
            f"Sine {ratio_sine:.3f} should have higher flute-band ratio than noise {ratio_noise:.3f}"
        )

    def test_low_tone_low_ratio(self):
        """A 100 Hz tone (below flute band) should have low ratio."""
        low = _sine_tone(100, seconds=3.0)
        ratio = _flute_band_ratio(low)
...
class TestValidate:
    ...
    def test_flags_flute_like_files(self, tmp_path: Path):
        ...
        # The flute-like one should be an outlier
        assert len(flagged) >= 1, "Should flag the flute-like background"
